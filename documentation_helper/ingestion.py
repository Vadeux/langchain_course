...
# Config SSL context to use certifi certificates
...
async def index_documents_async(
    ...
    log_info(
        f"📚 VectorStore Indexing: Preparing to add {len(documents)} documents to the vector store.",
        Colors.DARKCYAN,
    )
    ...
    if successful == len(batches):
        log_success(
            f"VectorStore Indexing: Successfully added all batches ({successful} / {len(batches)})."
        )
    else:
        log_warning(
...
async def main():
    """Main async func to orchestrate the entire process."""
    log_header("DOCUMENTATION INGESTION PIPELINE")

    log_info(
        "🗺️  TavilyCrawl: Starting to Crawl documentation from https://python.langchain.com/.",
        Colors.PURPLE,
    )
