...
def get_product_price(product: str) -> float:
    ...
    prices = {"laptop": 1299.99, "headphones": 149.99, "keyboards": 99.99}
...
def apply_discount(price: float, discount_tier: str) -> float:
    ...
    discount_percentages = {"bronze": 5.0, "silver": 10.0, "gold": 15.0}
    return round(price * (1 - (discount_percentages.get(discount_tier, 0.0) / 100)), 2)
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        print(f"    [Tool Selected] {tool_name} with args: {tool_input}")

        tool_to_call = tools_dict.get(tool_name)
        if tool_to_call is None:
            raise ValueError(f"Tool with name {tool_name} not found")
        observation = tool_to_call.invoke(tool_input)
